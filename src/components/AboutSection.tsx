...
export function AboutSection() {
  ...
  return (
    <section
      id="about"
      className="py-24 bg-gradient-to-b from-primary/[0.02] via-background to-secondary/[0.02] relative overflow-hidden"
    >
        ...
        <div className="grid gap-8 md:grid-cols-2 mb-20">
          {goals.map((goal, index) => {
            ...
            return (
              <Card
                key={index}
                className="border-0 shadow-xl bg-gradient-to-br from-card via-card to-muted/10 hover:shadow-2xl transition-all duration-300 group border border-border/20 backdrop-blur-sm"
              >
                <CardHeader className="text-center space-y-6 pb-6">
                  <div
                    className={`w-20 h-20 mx-auto bg-gradient-to-br ${goal.gradient} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform duration-300 border border-white/20`}
                  >
                  ...
                  <ul className="space-y-4">
                    {goal.items.map((item, itemIndex) => (
                      <li key={itemIndex} className="flex items-start gap-4 group">
                        <div
                          className={`w-3 h-3 mt-1.5 ${goal.bg} border-2 border-current rounded-full flex-shrink-0 ${goal.color}`}
                        />
